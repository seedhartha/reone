...
void CharacterGeneration::reloadCharacterModel() {
    ...
    _binding.portraitLbl->setBorderFill(_services.game.defaultPortraits().getTextureByAppearance(_character.appearance));
}
...
void CharacterGeneration::updateAttributes() {
    shared_ptr<CreatureClass> clazz(_services.game.defaultClasses().get(_character.attributes.getEffectiveClass()));
