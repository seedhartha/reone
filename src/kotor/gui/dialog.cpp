...
void DialogGUI::onStart() {
    ...
    auto &camera = _game.module()->area()->getCamera<AnimatedCamera>(game::CameraType::Animated);
...
void DialogGUI::updateCamera() {
    ...
    if (_dialog->cameraModel.empty()) {
        ...
        auto &camera = area->getCamera<DialogCamera>(game::CameraType::Dialog);
        ...
        auto &camera = area->getCamera<AnimatedCamera>(game::CameraType::Animated);
...
void DialogGUI::update(float dt) {
    ...
    if (_currentSpeaker && _game.cameraType() == game::CameraType::Dialog) {
        auto &camera = _game.module()->area()->getCamera<DialogCamera>(game::CameraType::Dialog);
