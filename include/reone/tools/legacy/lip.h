...
public:
    ...
    void toXML(IInputStream &lip, IOutputStream &xml);

private:
    void toXML(const std::filesystem::path &path, const std::filesystem::path &destPath);
    void toLIP(const std::filesystem::path &path, const std::filesystem::path &destPath);
};
